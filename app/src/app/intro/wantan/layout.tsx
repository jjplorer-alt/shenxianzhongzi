...
const SUB_NAV = [
  { label: "经文原文", href: "/intro/wantan/scripture", icon: BookOpen },
  { label: "经文注解", href: "/intro/wantan/commentary", icon: FileText },
  { label: "诵经音频", href: "/intro/wantan/audio", icon: Music },
];

export default function WantanLayout({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  const isIndex = pathname?.replace(/\/$/, "") === "/intro/wantan";
    ...
    <div className="min-h-screen">
      <div className="mx-auto max-w-3xl px-4 pt-6 pb-2">
        <nav className="flex items-center gap-1.5 text-[13px] text-muted-foreground">
          ...
          {isIndex ? (
            <span className="text-foreground/90">晚坛功课经</span>
          ) : (
            <Link href="/intro/wantan" className="transition-colors hover:text-gold">
              晚坛功课经
            </Link>
          )}
        </nav>
