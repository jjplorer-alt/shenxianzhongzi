...
const SUB_NAV = [
  { label: "经文原文", href: "/intro/zaotan/scripture", icon: BookOpen },
  { label: "经文注解", href: "/intro/zaotan/commentary", icon: FileText },
  { label: "诵经音频", href: "/intro/zaotan/audio", icon: Music },
];

export default function ZaotanLayout({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  const isIndex = pathname?.replace(/\/$/, "") === "/intro/zaotan";
    ...
    <div className="min-h-screen">
      <div className="mx-auto max-w-3xl px-4 pt-6 pb-2">
        <nav className="flex items-center gap-1.5 text-[13px] text-muted-foreground">
          ...
          {isIndex ? (
            <span className="text-foreground/90">早坛功课经</span>
          ) : (
            <Link href="/intro/zaotan" className="transition-colors hover:text-gold">
              早坛功课经
            </Link>
          )}
        </nav>
